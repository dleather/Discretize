function [Z,Zprob] = tauchenVAR(N,mu,rho,sigma,m)
%Function TAUCHEN
%
%Purpose:    Finds a Markov chain whose sample paths
%            approximate those of the K-dimensional VAR(1) process
%                z(t+1) = (1-rho)*mu + rho * z(t) + eps(t+1)
%            where eps are multi-normal with stddev sigma
%
%Format:     {Z, Zprob} = Tauchen(N,mu,rho,sigma,m)
%
%Input:      N       scalar, number of nodes for Z
%            mu      (K x 1), unconditional mean of process
%            Phi     (K x K)
%            Sigma   (K x K), sqrtm(VCOV) and std = diag(Sigma)
%            m       max +- std. devs.
%
%Output:     Z       (N x K) matrix, nodes for Z
